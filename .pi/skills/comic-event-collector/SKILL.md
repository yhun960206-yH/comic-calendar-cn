...
仅处理与漫展、同人展、动漫游戏展、兽展直接相关的中国城市实体活动；不将一般游戏节、长期主题咖啡馆或纯文创商贸会混充漫展；童书展、出版展衍生的“漫画周”类项目也不属于本日历，除非用户明确要求收录。称呼以官方类型为准。目标是尽量收录能核对的活动，**不是**宣称全国完整覆盖。用户无需亲自逐条录入，AI 做检索和字段核验；但 AI 的核验不等于主办方保证或现场人工核实。

### 来源分级（按可信度递减，逐条在证据里标明层级）

1. **主办方官网／政府或场馆公示**：最高。
2. **主办方认证账号公告的媒体镜像**：需镜像页能直接访问、且显示认证主体名称；在 `attribution` 写明“原站未直访”。
3. **聚合站／本地信息站列表（如本地宝、活动网、喵特）**：只有第二条独立来源在名称、日期、场馆上完全一致时才可用，并在 `attribution` 说明该层级的弱点。单一聚合来源一律隔离。
4. **仅搜索结果摘要、短视频文案、预测排期表**：不可用。

### 已接入的平台来源（与上面的按需采集不同）

`src/sources/bili_channel.py` 是**随发布工作流每日运行的 B 站会员购频道适配器**，其风险与机制写在 README「B 站会员购漫展源」一节。要点：接口为站点内部接口、无授权路径、需设备指纹 cookie 才能正确分页、会把平台数据公开再分发。因此：

- 不要把它当作可以照搬的「官方 API」，也不要在其它项目里因为「别人也在抓」而复制这种做法。
- 该来源的字段只取短事实；不要在 Skill 里为它补充图片、详情正文或票档。
- 若它抓到的活动与 Skill 人工核对的结果冲突，以主办方公开公告为准，并按仓库规则递增 `revision`。
- 适配器可被移除（去掉 `--bili`），不要把它写成不可替代的依赖。

### 已知不可用或需额外条件

- **B 站、抖音没有可自助申请的活动列表接口**；哔哩哔哩开放平台不提供会员购展会目录，抖音生活服务能力面向商家／服务商且需行业角色认证与问卷审批。这是本项目改用站内频道适配器的原因，也说明其为已接受的运营方风险而非授权采集。
- robots 未发布（`show.bilibili.com` 错误页、`yunmanzhan.com` 404）**不等于**获得许可；`koyomi.today`、`dmzhan.com` 当前不可达。
- 镜像页可被搜索引擎收录并不代表可被直接复现访问；采集时若无法重新取得正文，就不要拿它当 `source_url`（本轮沈阳 SSCA 即因此隔离）。
- 场馆“挂 A 市名、实际在 B 市”（如西安临空会展中心在咸阳渭城区）需先确认城市归属规则再发布。
...
2. 用多组不同城市/月份/主办方词检索。优先主办方官网的**公开活动公告**，再用市政府、官方场馆或具名主办方的其他公开网页交叉核对场馆地址。官网找不到时，可查证公开可访问、明确标注主办方认证身份的正规媒体镜像公告（保留镜像 URL，并注明未经直访原平台），结合独立的场馆地址来源；不得仅靠搜索结果摘要或聚合站点的预测排期发布。查看 robots/站点条款；不绕过登录、验证码、限流、私有接口。微博通用 robots 禁抓、B 站/抖音未经确认的自动取数权限均不能用“其他人也在抓”替代许可。仅整理公开的短事实字段与原始 URL；不拷贝原创文字、海报、图片或票务数据。批量导入第三方数据库必须先确认其数据再发布条件。
...
4. 对确认为独立活动的候选分配稳定小写 ASCII `event_id`（如 `ai-cicf-agf-guangzhou-2026`）；同一展会不同公告不能建多个 UID。核对线上/本地是否已经有相同主办方、标题、城市、日期或来源 URL 的记录。字段必须遵循 `src/model.py`：`city_code` 在 `config/cities.json` 内，行政细分对应 `config/area_to_city.json`，`end_date` 是**最后一天的次日**；没有具体开闭馆时间就使用全天，不填 `start_at/end_at`；`updated_at` 为本次查阅的 UTC 时间，不表示人工实地核验。票务、地图、嘉宾不确定则不填。`source_url` 优先可访问的官方 HTTPS 页面；如主办方仅通过平台发布且原站不允许访问，可使用公开允许读取、保留其认证主办方署名的正规媒体镜像 HTTPS 页面，在 attribution/证据中明确镜像身份和原站未直访。其他佐证写证据文件而不是编造按钮链接。
